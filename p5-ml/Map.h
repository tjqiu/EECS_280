...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
        ...
        Pair_type find_pair = {k, value};
        return bst.find(find_pair);
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        if (find(k) == end()) {
            Value_type value = Value_type();
            Pair_type pair = {k, value};
            std::pair<Iterator, bool> ins_ret = insert(pair);
            Pair_type & ret = *(ins_ret.first);
            return ret.second;
        }
        else {
            Pair_type & ret = *find(k);
            return ret.second;
        }
    }
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        if (find(val.first) != end()) {
            std::pair<Iterator, bool> ret = {find(val.first), false};
            return ret;
        }
        else {
            Iterator retI = bst.insert(val);
            std::pair<Iterator, bool> ret = {retI, true};
            return ret;
        }
    }
