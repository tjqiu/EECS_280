...
TEST(map_public_test) {
  ...
  ASSERT_EQUAL(words["hello"], 1);
   auto p = *(words.begin());
    ASSERT_EQUAL(p.second, 1);
    ASSERT_EQUAL(p.first, "hello");
    auto end = words.end();
    ASSERT_EQUAL(end, words.find("ss"));
  ...
  ASSERT_EQUAL(words["world"], 2);
    auto k = *(words.begin());
     ASSERT_EQUAL(k.second, 1);
     ASSERT_EQUAL(k.first, "hello");
    ASSERT_EQUAL(words.size(), 2);
    
    pair<string, double> tuple2;
    tuple2.first = "world";
    tuple2.second = 10;
    auto o = words.insert(tuple2);
    ASSERT_EQUAL(o.second, false);
    ASSERT_EQUAL((*(o.first)).first, "world");
    ASSERT_EQUAL((*(o.first)).second, 2);
